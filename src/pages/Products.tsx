...
// small style for spinner animation
const spinnerStyle = `
...
export default function Products() {
  ...
  function normalizeProduct(raw) {
    ...
    const category =
      catRaw && typeof catRaw === "object"
        ? { id: catRaw.id ?? catRaw._id, name: catRaw.name ?? catRaw.title ?? "" }
        : catRaw ?? null;
    let imagesArr = [];
    if (Array.isArray(raw.images)) imagesArr = raw.images;
    else if (Array.isArray(raw.extra_images)) imagesArr = raw.extra_images;
  ...
  // open add drawer
  const openAddDrawer = () => {
    // prefetch drawer/editor for snappy UX
    void import("./ProductDrawer");
    void import("./Editor");
  ...
  // open edit drawer (fetch fresh data)
  const openEditDrawer = async (p) => {
    try {
      // prefetch drawer/editor on intent
      void import("./ProductDrawer");
      void import("./Editor");
      ...
      const res = await api.get(`${basePath}/show/${p.id}`);
      const body = res.data ?? res;
      const prodRaw = body.data ?? body.product ?? body;
  ...
  const openView = (p) => {
    // prefetch editor in case user chooses to edit description from view
    void import("./Editor");
    setSelectedProduct(p);
    setIsViewOpen(true);
    setTimeout(() => viewDrawerRef.current?.focus(), 120);
  ...
  // Image cell uses Suspense with a small placeholder
  const ImageCell = ({ p }) => (
    <div className="w-12 h-12 rounded-full overflow-hidden bg-white border">
      <Suspense fallback={<div className="w-full h-full bg-slate-100" />}>
        <ImageWithFallback src={resolveImage(p)} alt={p.name ?? "product"} className="object-cover w-full h-full" />
      </Suspense>
    </div>
  );

  // Prefetch helpers for hover
  const prefetchDrawer = () => void import("./ProductDrawer");
  const prefetchEditor = () => void import("./Editor");
  const prefetchImageWithFallback = () => void import("./ImageWithFallback");
  ...
  return (
    <>
      ...
      <div className="w-full px-4 md:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 w-full">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <Button
              ...
              onMouseEnter={() => {
                // small prefetch (category drawer is light but still)
              }}
              className="ml-2"
            >
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {isLoading ? (
                  ...
                  products.map((p, i) => (
                    <tr key={String(p.id)}>
                      <td className="px-4 py-3 text-sm align-middle">{i + 1}</td>
                      <td className="px-4 py-3 align-middle"><ImageCell p={p} /></td>
                      <td className="px-4 py-3 align-middle">
                        <div className="text-sm font-medium truncate max-w-[220px] sm:max-w-none">{p.name}</div>
                        <div className="text-xs text-slate-400 mt-1">{p.slug ?? ""}</div>
                      </td>
        ...
        {/* Drawer for add/edit product — lazy and wrapped in Suspense */}
