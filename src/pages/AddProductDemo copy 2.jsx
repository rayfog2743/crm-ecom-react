...
const SAMPLE_COLORS = [
  { id: 1, name: "Red", hex: "#FF0000", image: "/mnt/data/e5ef9766-71d3-400b-ac61-2e346bc31dc7.png" },
  { id: 2, name: "Blue", hex: "#0000FF", image: "/mnt/data/e5ef9766-71d3-400b-ac61-2e346bc31dc7.png" },
  { id: 3, name: "Green", hex: "#00FF00", image: "/mnt/data/e5ef9766-71d3-400b-ac61-2e346bc31dc7.png" },
];
...
const fetchvariations = async () => {
  ...
  const handleMainImage = (file) => { if (!file) return; setMainImage({ file, url: URL.createObjectURL(file) }); };
  const handleAddImages = (files) => { const mapped = Array.from(files).map((f) => ({ file: f, url: URL.createObjectURL(f) })); setAdditionalImages((prev) => [...prev, ...mapped]); };
  const removeAdditionalImage = (idx) => setAdditionalImages((prev) => prev.filter((_, i) => i !== idx));
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (mainImage) {
      if (mainImage.file instanceof File) {
        ...
        const file = await urlToFile(mainImage.url, fname, mainImage.file?.type || 'image/jpeg');
    ...
    try {
      ...
      console.log('formData.getAll gallery[] not supported in this environment');
    }
    console.log('formData keys:');
            ...
            <div className="grid grid-cols-3 gap-3">
              {additionalImages.length === 0 ? (
                ...
                additionalImages.map((a, i) => (
                  <div key={a.url + i} className="relative border rounded overflow-hidden">
                    <img src={a.url} alt={`gallery-${i}`} className="w-full h-24 object-cover" draggable={false} />
  ...
  function renderStep() {
    switch (step) {
      case 0:
        return (
          <div className="space-y-4">{/* Basic step */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input value={offerPrice} onChange={(e) => setOfferPrice(e.target.value)} className="mt-2 block w-full border rounded-lg p-3 shadow-sm" placeholder="Offer price (optional)" />
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mt-4">Plain Description</label>
              <textarea rows={3} value={plainDesc} onChange={(e) => setPlainDesc(e.target.value)} className="mt-2 block w-full border rounded-lg p-3 shadow-sm" placeholder="Short/plain description" />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mt-4">Big Description</label>
              <textarea rows={6} value={bigDesc} onChange={(e) => setBigDesc(e.target.value)} className="mt-2 block w-full border rounded-lg p-3 shadow-sm" placeholder="Enter detailed/full product description" />
            </div>
          </div>
        );
      ...
      case 2:
        return (
          <div className="space-y-4">{/* Variations */}
            <div className="flex items-center justify-between mb-3">
            ...
            <div className="grid gap-4">
              ...
              <div className="bg-white border rounded-lg p-4 shadow-sm">
                ...
                <div className="text-xs text-gray-500 mb-3">Configure per-variant price, SKU, stock and image. Inputs are responsive to available space.</div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200 text-sm" style={{ tableLayout: 'auto' }}>
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {variantRows.length === 0 && (
                        <tr><td colSpan={5} className="px-4 py-4 text-xs text-gray-400">No combinations yet (select options)</td></tr>
                      )}
                      ...
                      {variantRows.map((r, idx) => (
                        <tr key={r.key} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-4 align-top w-36">
                            <input type="number" value={r.priceExtra} onChange={(e) => updateVariantRow(r.key, { priceExtra: e.target.value })} className="px-2 py-2 border rounded w-full" placeholder="Extra" />
                          </td>

                          <td className="px-4 py-4 align-top w-40">
                            <input type="text" value={r.sku} onChange={(e) => updateVariantRow(r.key, { sku: e.target.value })} className="px-2 py-2 border rounded w-full truncate" placeholder="SKU" />
                          </td>

                          <td className="px-4 py-4 align-top w-32">
                            <input type="number" value={r.qty} onChange={(e) => updateVariantRow(r.key, { qty: e.target.value })} className="px-2 py-2 border rounded w-full" placeholder="Qty" />
                          </td>

                          <td className="px-4 py-4 align-top w-36">
                            <div className="flex items-center gap-3">
                              <label className="cursor-pointer px-2 py-1 bg-white border rounded text-xs">
                                <input type="file" accept="image/*" className="hidden" onChange={(e) => handleVariantImage(r.key, e)} />
                                Upload
                              </label>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Review generated combinations below.</p>
              </div>
            </div>
            ...
            <div className="bg-white border rounded-lg p-4 shadow-sm">
              ...
              <div className="overflow-x-auto">
                <table className="w-full table-fixed border-collapse text-sm">
                  ...
                  <tbody>
                    {variantRows.length === 0 && <tr><td colSpan={5} className="p-3 text-xs text-gray-400">No variants configured</td></tr>}
                    {variantRows.map((r) => (
                      <tr key={r.key} className="align-top">
                        ...
                        <td className="p-2 border text-right">₹{Number(price || 0) + Number(r.priceExtra || 0)}</td>

                        <td className="p-2 border">{r.sku || '—'}</td>

                        <td className="p-2 border text-right">{r.qty || '—'}</td>

                        <td className="p-2 border text-center">{r.image ? <img src={r.image.url} alt="v" className="w-16 h-10 object-cover rounded border" /> : <div className="text-xs text-gray-400">No photo</div>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-4 flex items-center justify-end gap-3">
                <button onClick={handleSubmit} className="px-4 py-2 bg-indigo-600 text-white rounded">Submit product</button>
              </div>
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                ...
                <div className="mt-6 flex items-center justify-between">
                  <div>
                    ...
                    {step < 4 && <button type="button" onClick={next} disabled={!canNext()} className={`px-4 py-2 rounded-lg ${canNext() ? "bg-indigo-600 text-white" : "bg-gray-200 text-gray-400"}`}>Next</button>}
                  </div>

                  <div>
                    <button type="button" onClick={() => { setSubmittedPayload(null); setName(""); setPrice(""); setOfferPrice(""); setPlainDesc(""); setSelectedPerGroup({}); setMainImage(null); setAdditionalImages([]); setVariantRows([]); setCategory(""); setBigDesc(""); setVideoUrl(""); setSeoTitle(""); setSeoDescription(""); setSeoKeywords([]); setStep(0); }} className="px-4 py-2 bg-gray-100 rounded-lg mr-2">Reset</button>
                    <button type="button" onClick={() => setStep(4)} className="px-4 py-2 bg-white border rounded-lg">Go to Review</button>
                  </div>
                </div>
              </form>
            </div>
